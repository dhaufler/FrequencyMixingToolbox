function [All_R1,All_R2] = Get_all_possible_roots(X,Y,RelType,Centroid_mag, TripletTable)
%INPUT:
%       X, Y:       vector of X,Y coordinates of peaks
%       RelType:    vector of Triplet type (1-8)

%init
N_clusters = length(X);%total number of XYZ centroids for the microstate
pCluNumbers = cell(N_clusters,1); %cell array of possible triplet numbers
R1_X = TripletTable.XY_R1(:,1); %coefficient of X in the determining R1, where rows are triplet types
R1_Y = TripletTable.XY_R1(:,2); %coefficient of X in the determining R1, where rows are triplet types
R2_X = TripletTable.XY_R2(:,1); %coefficient of X in the determining R2, where rows are triplet types
R2_Y = TripletTable.XY_R2(:,2); %coefficient of X in the determining R2, where rows are triplet types
...
for j=1:length(X) %for each XYZ centroid
    ...
    pCluNumbers{j} = find(TripletTable.Relationship_Type==RelType(j));
end
AllTypes = cartesianProduct(pCluNumbers); 

%expand X and Y to the dimension of 'AllTypes'
if size(X,1)>1
    X=X';
end
if size(Y,1)>1
...
X_expanded = repmat(X,[size(AllTypes,1),1]);
Y_expanded = repmat(Y,[size(AllTypes,1),1]);
All_R1 = X_expanded.*R1_X(AllTypes) + Y_expanded.*R1_Y(AllTypes);
All_R2 = X_expanded.*R2_X(AllTypes) + Y_expanded.*R2_Y(AllTypes);

% %Now, get every putative R1 and R2 for each row of the table
%     %init 
